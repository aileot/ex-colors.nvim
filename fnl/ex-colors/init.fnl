...
(fn compose-hi-cmd-lines [ex-colors-name]
  "Compose hi cmd lines with vim.g.colors_name for `ex-colors-name`.
  ...
  (let [file-ext :lua
        ...
        colors-name-line (gvar-template:format :colors_name
                                               (.. "\"" ex-colors-name "\""))
        hi-cmd-lines (compose-hi-cmdlist)
        new-lines (-> [colors-name-line gvar-support-lines hi-cmd-lines]
                      (flatten))]
    new-lines))
...
(fn generate-hi-cmds []
  (let [file-ext :lua
    ...
    (let [cmd-lines (compose-hi-cmd-lines ex-colors-name)
...
{: setup : compose-hi-cmdlist : generate-hi-cmds}
